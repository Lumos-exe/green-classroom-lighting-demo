...
\title{\heiti 基于三维工作表面单元的纯视觉教室精细照明调控方法研究}
\author{作者：刘泽培、周北辰、王心桐\\专业：计算机科学与技术（拔尖班）\\课程：绿色低碳校园}
\date{}
...
\end{equation}
这些系数不再完全由人工设定，而是通过人工标注的最终灯光控制策略进行离线校准。对于若干典型课堂场景，由人工给出满足大学教室照明设计要求并兼顾节能偏好的期望灯光控制向量。本文采用的主要照明要求如表 \ref{tab:lighting-reference} 所示，其中大学教室照明设计以上位标准 GB/T 50034-2024 为依据，中小学校教室相关标准仅作为相似视觉任务的参考\cite{gbt50034-2024,gb7793-2010,gb7793-2025,gbt36876-2018}。

\begin{table}[H]
  \centering
  \small
  \caption{人工标注期望控制策略参考的主要照明要求}
  \label{tab:lighting-reference}
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.34\linewidth}>{\raggedright\arraybackslash}p{0.20\linewidth}>{\raggedright\arraybackslash}X}
    ...
    指标 & 建议要求 & 依据定位 \\
    \midrule
    课桌面维持平均照度 & $\geq 300\,\mathrm{lx}$ & 大学普通教室读写工作面的目标照度，参考普通教室视觉任务要求 \\
    黑板或书写板维持平均照度 & $\geq 500\,\mathrm{lx}$ & 板书和观看区域的目标照度，约束黑板/白板相关工作表面 \\
    工作面照度均匀度 & 课桌面 $\geq 0.7$；书写板 $\geq 0.8$ & 用于避免关键工作表面局部过暗或明暗差异过大 \\
    眩光限制 & UGR $\leq 19$ 或更严格 & 结合建筑照明设计要求控制直射眩光和过亮区域 \\
    色温与显色性 & $3300\sim 5300\,\mathrm{K}$；$R_a\geq 80$ & 作为课堂阅读、书写和板书观看的基础视觉质量要求 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
本文采用原型仿真验证，验证重点是检查方法链条是否闭合、各变量是否能够在统一 $cell_i$ 索引下工作，以及控制优化是否能够根据归一化目标需求给出合理灯光方案。代码实现中预留了官方 VGGT 与 3DGS 的接口位置，用于说明真实重建模块在系统中的输入输出关系；但本文当前报告的数值结果来自合成点图、合成多相机观测和规则化感知 head，模拟课堂书写、课间讨论、投影、板书和自习五类课堂状态。该验证不运行真实 VGGT/3DGS 权重，不使用真实灯具等硬件，因此结果只表示方法流程和变量关系的可执行性，不代表真实部署性能。真实教室数据采集、硬件接口和长期运行评价不属于本文验证范围。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.20\linewidth}>{\raggedright\arraybackslash}X}
    ...
    项目 & 内容 \\
    \midrule
    输入数据 & 当前数值结果使用合成 VGGT-like 点图、合成多相机观测和规则化感知输出；代码保留 VGGT/3DGS 接口位置，但本文不使用真实多相机采集数据 \\
...
图 \ref{fig:quality-metrics} 汇总了原型仿真中的任务相关欠照、亮度上限过照、相对能耗估计和任务满足率。整体上，五类场景在设定容差内的任务满足率均达到 1.000，任务相关平均欠照保持在较低水平。class writing 场景覆盖课桌、教师桌和讲台等大范围工作表面，相对能耗估计最高，为 0.823；break discussion 场景考虑课间走动、讨论以及可能存在的短时书写需求，相对能耗估计为 0.799，接近上课模式；projection 场景需要降低投影区域强光，相对能耗估计最低；board writing 与 self study 场景分别体现前区板书照明和局部自习照明。该结果符合本文方法的预期：控制器不是单独最大化节能，也不是简单保持全教室高亮，而是在满足任务表面需求的前提下，根据场景活动范围进行相对节能控制。
...
\begin{table}[H]
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    课堂书写 & 0.003 & 0.007 & 0.823 & 1.000 & 0.339 \\
    课间讨论 & 0.014 & 0.001 & 0.799 & 1.000 & 0.339 \\
    投影 & 0.000 & 0.000 & 0.266 & 1.000 & 0.041 \\
    板书 & 0.004 & 0.005 & 0.623 & 1.000 & 0.254 \\
    自习 & 0.000 & 0.000 & 0.454 & 1.000 & 0.134 \\
    \bottomrule
  \end{tabular}
\end{table}

综合来看，原型仿真验证表明，本文提出的变量链路能够在代码层面闭合：工作表面 cell 能够承载占用、活动、当前光照、目标需求和灯具贡献；灯具贡献矩阵能够把控制变量映射到工作表面光照；目标需求函数能够把课堂活动转化为控制目标；约束优化能够输出满足边界约束的灯具控制向量。

\section{研究结论}

本文围绕纯视觉条件下教室照明精细化调控问题，提出了一种以三维工作表面 cell 为核心索引的感知—需求—控制闭环方法。研究表明，教室照明控制不应仅依赖房间级占用判断、固定灯区划分或二维平面区域，而应首先面向真实教学任务建立三维工作表面表达。将课桌、黑板、投影幕、讲台和过道等区域离散为 $cell_i$ 后，人员占用、课堂活动、当前光照、环境光、目标需求和灯具贡献可以统一映射到同一物理坐标体系中，从而使“哪里需要光、为什么需要光、由哪些灯提供光”具备可计算、可解释和可优化的表达基础。

在控制逻辑上，本文证明了将课堂活动类别引入目标需求函数具有必要性。相比固定分区调光，仅根据“是否有人”进行控制难以区分书写、板书、投影、自习和课间讨论等不同教学状态。本文通过占用项、活动项、板书传播项和环境光分离机制，将课堂活动转化为不同工作表面的目标照明需求，并在人工期望控制策略中纳入照度、均匀度、眩光和显色性等国家标准要求，再结合灯具贡献矩阵进行约束优化。原型仿真结果表明，不同课堂场景能够生成差异化灯具控制向量：课堂书写和板书场景提高关键工作表面亮度，投影场景抑制投影区域过亮，自习场景则主要维持局部学习区域照明。这说明节能不应以牺牲关键任务表面的亮度为代价，而应来自对无人区域、低需求区域和过照区域的合理抑制。

原型仿真验证表明，本文提出的变量链路在代码层面可以闭合。工作表面 cell 能够承载占用、活动、当前光照、目标需求和灯具贡献等状态；灯具贡献矩阵能够将灯光控制变量映射到工作表面光照；目标需求函数能够根据课堂状态生成控制目标；约束优化能够在欠照、过照、能耗和平滑性之间进行折中，并输出满足边界约束的灯具控制向量。五类典型课堂场景中的任务满足率均达到设定要求，任务相关欠照保持在较低水平，相对能耗也随场景需求范围发生变化，说明该方法具备从视觉感知到灯光控制的基本可执行性。

需要指出的是，本文当前结果仍属于方法设计与原型仿真验证，不能直接等同于真实教室部署效果。由于实验使用的是合成点图、合成多相机观测和规则化感知输出，尚未运行真实 VGGT、3DGS 和 Swin-Tiny-FPN 权重，也未完成灯具功率测量和长期用户舒适度评价，因此文中的相对能耗、任务满足率和欠照/过照指标只能说明方法链路和变量关系的合理性，不能直接解释为真实节能率或真实照明质量结论。后续研究应进一步采集真实多相机大学教室数据，补充语义标注、灯具贡献矩阵实测、照度计对照实验、灯具功率曲线建模和用户视觉舒适度评价，从而将本文方法从原型验证推进到真实工程验证阶段。
